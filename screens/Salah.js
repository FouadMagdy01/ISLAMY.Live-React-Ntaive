    ...
    const requestLocationPermission = async () => {
        try {
          ...
          if (granted === PermissionsAndroid.RESULTS.GRANTED) {
            ...
            setLoading(true)
            const response = await axios.get(`https://api.aladhan.com/v1/calendar?latitude=${latitude}&longitude=${longitude}`)
            setData(response.data.data)
            const initialSlide = new Date().getDate()
            setToday(initialSlide-1)
            setLoading(false)
            } else {
                setGranted(false)
          }
